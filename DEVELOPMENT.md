...
Bump the version in `package.json`, `package-lock.json` and the JSON-LD block of `index.html`. The CDN lines use
`@latest` and need no bump. Date the CHANGELOG entry, commit `chore: release x.y.z`, run both test suites, `npm publish`, tag
