...
// takes a function for text that changes. Add label="Copy" for a labelled button. The icon turns into a check for
// two seconds, and the label into "Copied".
const ICONS = {
...
class CopyButton extends HTMLElement {
  ...
  connectedCallback() {
    ...
    this.innerHTML = `<button type="button" class="${label ? 'button' : 'copy-icon'}" aria-label="Copy to clipboard">
      <svg viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round"/></svg>${label ? '<span aria-live="polite"></span>' : ''}</button>`
  ...
  #show(copied) {
    ...
    if (label) label.textContent = copied ? 'Copied' : this.getAttribute('label')
  }
